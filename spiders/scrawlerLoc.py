...
class CompanyLocSpider(scrapy.Spider):
    name = "scrawlerLoc"
    start_urls = [
    ...
    def parse_info_page(self, response):
        for title in response.css("ul.company_list div.company_list_text"):
            ...
            yield scrapy.Request(title.css("a::attr(href)").extract_first(), self.parse_loc)
    ...
    def parse_loc(self, response):
        ...
        yield {
            "loc": response.css("div.xq_gsxx span.addR")[2].css("span::text").extract_first()
        }